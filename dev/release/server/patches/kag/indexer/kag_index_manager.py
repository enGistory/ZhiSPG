...
@KAGIndexManager.register("tender_document_index")
class TenderDocumentIndexManager(KAGHybridIndexManager):
    @property
    def name(self):
        return "招标文件索引管理器"
    ...
    @property
    def description(self) -> str:
        return "面向招标文件的领域索引，抽取评分项、评分点、法规依据、金额、项目名、编号、单位等关键信息，并保留与原文文本块的关联，便于招标文件审查、评分规则定位和金额/单位核验。"
...
TenderScoringItem(评分项): IndexType
    ...
    @property
    def index_cost(self) -> str:
        return "复用 KAG 混合抽取与召回链路，成本接近基于文本块和图谱的混合索引；适合在招标文件需要结构化抽取时使用。"
    ...
    @property
    def retrieval_method(self) -> str:
        return "面向招标文件字段构建领域图谱索引，并结合文本块召回定位原文依据。"
    ...
    def build_extractor_config(
        ...
        return [
            {
                "type": "tender_document_extractor",
                "kag_qa_task_config_key": kb_task_project_id,
            }
        ]
    ...
    def build_retriever_config(
        ...
        return ChunkIndexManager.build_retriever_config(
            llm_config, vectorize_model_config, **kwargs
        )


@KAGIndexManager.register("bid_document_index")
class BidDocumentIndexManager(KAGHybridIndexManager):
    @property
    def name(self):
        return "投标文件索引管理器"
    ...
    @property
    def description(self) -> str:
        return "面向投标文件的领域索引，抽取项目特色（技术方案先进）、施工组织设计拆分、单位等关键信息，并保留与原文文本块的关联，便于投标响应分析和技术方案亮点检索。"
...
ConstructionDesignPart(施组设计拆分): IndexType
    ...
    @property
    def index_cost(self) -> str:
        return "复用 KAG 混合抽取与召回链路，成本接近基于文本块和图谱的混合索引；适合在投标文件需要结构化抽取时使用。"
    ...
    @property
    def retrieval_method(self) -> str:
        return "面向投标文件字段构建领域图谱索引，并结合文本块召回定位原文依据。"
