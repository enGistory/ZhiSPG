...
class EmbeddingVectorManager(object):
    ...
    def _generate_dense_vectors(self, dense_vectorizer, text_batch, batch_size=10):
        batch_size = min(int(batch_size or 10), 10)
        ...
        for idx in range(n_batchs):
            ...
            if len(sub_texts) == 0:
                continue
            vectors = dense_vectorizer.vectorize(sub_texts)
            if vectors is None:
                raise RuntimeError(
                    f"dense vectorizer returned None for batch size {len(sub_texts)}"
                )
            embeddings.extend(vectors)
        return embeddings

    def _generate_sparse_vectors(self, sparse_vectorizer, text_batch, batch_size=10):
        batch_size = min(int(batch_size or 10), 10)
        texts = list(text_batch)
        if not texts:
            return []
        ...
        for idx in range(n_batchs):
            ...
            if len(sub_texts) == 0:
                continue
            vectors = sparse_vectorizer.vectorize(sub_texts)
            if vectors is None:
                raise RuntimeError(
                    f"sparse vectorizer returned None for batch size {len(sub_texts)}"
                )
            embeddings.extend(vectors)
        return embeddings

    async def _agenerate_dense_vectors(
        self, dense_vectorizer, text_batch, batch_size=10
    ):
        batch_size = min(int(batch_size or 10), 10)
        texts = list(text_batch)
        if not texts:
    ...
    async def _agenerate_sparse_vectors(
        self, sparse_vectorizer, text_batch, batch_size=10
    ):
        batch_size = min(int(batch_size or 10), 10)
        texts = list(text_batch)
        if not texts:
    ...
    def batch_generate_dense(self, dense_vectorizer, batch_size=10):
    ...
    def batch_generate_sparse(self, sparse_vectorizer, batch_size=10):
    ...
    async def abatch_generate_dense(self, dense_vectorizer, batch_size=10):
        text_batch = self._get_dense_text_batch()
    ...
    async def abatch_generate_sparse(self, sparse_vectorizer, batch_size=10):
        text_batch = self._get_sparse_text_batch()
...
class EmbeddingVectorGenerator(object):
    ...
    def batch_generate(self, node_batch, batch_size=10):
        batch_size = min(int(batch_size or 10), 10)
    ...
    async def abatch_generate(self, node_batch, batch_size=10):
        batch_size = min(int(batch_size or 10), 10)
        manager = EmbeddingVectorManager(self._disable_generation)
        dense_vec_meta, sparse_vec_meta = self._vector_index_meta
        for node_item in node_batch:
...
class BatchVectorizer(VectorizerABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.batch_size = min(int(batch_size or 10), 10)
